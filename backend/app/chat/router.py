...
async def update_conversation(
    body: UpdateConversationRequest, user: dict = Depends(get_current_user)
):
...
async def save_message(body: SaveMessageRequest, user: dict = Depends(get_current_user)):
...
async def get_messages(
    conversation_id: PydanticObjectId, user: dict = Depends(get_current_user)
):
